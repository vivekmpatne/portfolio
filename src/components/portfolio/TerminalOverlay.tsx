...
function ShellPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-x-2 bottom-2 top-16 z-50 flex flex-col border border-[var(--phosphor)]/70 bg-[#05100a] shadow-[0_0_60px_-10px_var(--phosphor)] backdrop-blur-md md:inset-x-auto md:right-4 md:bottom-4 md:top-auto md:h-[540px] md:w-[520px]">
      {/* Title bar */}
      <div className="flex items-center justify-between border-b border-[var(--phosphor)]/40 bg-black/80 px-3 py-1.5">
        <div className="flex items-center gap-1.5">
          <span className="h-2.5 w-2.5 rounded-full bg-red-500/80" />
          <span className="h-2.5 w-2.5 rounded-full bg-amber-500/80" />
          <span className="h-2.5 w-2.5 rounded-full bg-green-500/80" />
        </div>
        <div className="text-[11px] uppercase tracking-widest text-[color:var(--phosphor)]/70">
          guest@vivek:~ — {created}
        </div>
        <button
          type="button"
          onClick={onClose}
          className="text-[color:var(--phosphor)]/70 hover:text-[color:var(--phosphor)]"
